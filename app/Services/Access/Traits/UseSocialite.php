...
trait UseSocialite
{
    ...
    protected function getSocialLinks()
    {
        ...
        if (strlen(getenv('WEIXIN_KEY'))) {
            $socialite_enable[] = link_to_route('auth.provider', trans('labels.frontend.auth.login_with', ['social_media' => 'Weixin']), 'weixin');
        }
        if (strlen(getenv('QQ_KEY'))) {
            $socialite_enable[] = link_to_route('auth.provider', trans('labels.frontend.auth.login_with', ['social_media' => 'QQ']), 'qq');
        }
        if (strlen(getenv('WEIXINWEB_KEY'))) {
            $socialite_enable[] = link_to_route('auth.provider', trans('labels.frontend.auth.login_with', ['social_media' => 'WeixinWeb']), 'weixinweb');
        }
